...
function drawGameStats() {
    ...
    text(`Players: ${guests.length}`, statsX, statsY + lineHeight * 10);

    if (partyIsHost()) {
        text(`${me.playerName},${me.playerNumber} (HOST): ${me.status}`, statsX, statsY + lineHeight * 11);
    } else {
        text(`${me.playerName},${me.playerNumber}: ${me.status}`, statsX, statsY + lineHeight * 11);
    }


    if (twoPlayersWithTheSamePlayerNumberExist()) {
        text(`Two players have spawn with the same playerNumber`, statsX, statsY + lineHeight * 12);
        text(`One of them must refresh the browser.`, statsX, statsY + lineHeight * 13);
    } else {
        text(`All players have unique playerNumbers`, statsX, statsY + lineHeight * 12);
    }
